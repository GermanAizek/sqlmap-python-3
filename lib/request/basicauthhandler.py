    ...
    def http_error_auth_reqed(self, auth_header, host, req, headers):
        ...
        if hash(req) not in self.retried_req:
            ...
            if self.retried_count > 5:
                raise urllib.error.HTTPError(req.get_full_url(), 401, "basic auth failed", headers, None)
            else:
                self.retried_count += 1

        return urllib.request.HTTPBasicAuthHandler.http_error_auth_reqed(self, auth_header, host, req, headers)
